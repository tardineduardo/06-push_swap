...
/*   Updated: 2024/11/13 16:32:55 by eduribei         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../push_swap.h"

static void	ft_set_costs_of_moves(int di_s_head, int di_d_head, t_dll *node)
{
	if (di_s_head > 0 && di_d_head > 0)
	{
		node->move_rev = true;
		node->cost_rev = 1 + ft_abs(di_s_head) + ft_abs(di_d_head - di_s_head);
	}
	else if (di_s_head < 0 && di_d_head < 0)
	{
		node->move_rot = true;
		node->cost_rot = 1 + ft_abs(di_s_head) + ft_abs(di_d_head - di_s_head);
	}
	else if (di_s_head <= 0 && di_d_head >= 0)
	{
		node->move_opo_srot_drev = true;
		node->cost_opo_srot_drev = 1 + ft_abs(di_s_head) + ft_abs(di_d_head);
	}
	else if (di_s_head >= 0 && di_d_head <= 0)
	{
		node->move_opo_srev_drot = true;
		node->cost_opo_srev_drot = 1 + ft_abs(di_s_head) + ft_abs(di_d_head);
	}
}
