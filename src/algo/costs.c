...
//FROM B TO A
static int	calculate_b(t_info *s, t_dll *node)
{
	...
	if (node->value < (s->t_len / 2) || s->b_len < 3)
		return (999);
	dest_a_dist = ft_dclst_dist_head_bidi(&(s->a), s->hi_a->next);
	node_b_dist = ft_dclst_dist_head_bidi(&(s->b), node);

	trav_a = s->hi_a;
	if (trav_a->value < node->value)
	{
		node->cost = ft_dclst_dist_head_bidi(&(s->a), s->hi_a->next);
	...
	{                 								
		...
		&& trav_a->prev->value != s->hi_a->value)
			trav_a = trav_a->prev;
		node->cost = abs(node_b_dist) + abs(ft_dclst_dist_head_bidi(&(s->a), trav_a));
		s->a_to_move = trav_a;
	}
}
...
// COST FROM A->B
static int	calculate_a(t_info *s, t_dll *node)
{
